...
function MODEL_HTML(){
  ...
  <div class="rgrid">
    ...
    ${card("Team ratings and line flags","flags, not picks","m",`Every team gets a value (Teams tab) from four opinions: the books' rating from past lines, the on-field stats (offense, defense and special teams, each measured against opponent quality), this week's quarterback, and, when the refresh can reach it, the published power rankings at 10%. Two fair lines per game: the ratings (everything) and the stats (no books). Since 2016, each season rated only from earlier ones: when DraftKings' spread sat 5+ points from the stats, the side the stats liked covered 57.0% (344 games, 9 of 10 seasons); 6+ points, 60.7% (201 games, 10 of 10). Ratings 3+ points off: 53.0% (843 games, 8 of 10). The thresholds were chosen after seeing the results, so the true rates are likely lower. Team-by-team home field was tested and didn't carry over from season to season, so every team gets the same 2.3 points.`)}
